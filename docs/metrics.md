...
|--------------------|-------------------|----------------------------------------------------------------------------------------------------|
| `request_duration` | Timer (Histogram) | The time taken by each request.                                                                    |
| `request_status`   | Counter           | The number of requests completed, tagged by their status code classification (`2xx`, `4xx`, `5xx`) |

The `request_status` counter has a tag, `status` with the following values:
