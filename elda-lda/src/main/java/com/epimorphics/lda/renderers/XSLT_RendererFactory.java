...
public class XSLT_RendererFactory implements RendererFactory {
    ...
    @Override
    public Renderer buildWith(final APIEndpoint ep, final ShortnameService sns) {
        return new Renderer() {
            ...
            @Override
            public Renderer.BytesOut render(Times t, Bindings rc, Map<String, String> termBindings, APIResultSet results) {
            ...
            public void handleMetadata(APIResultSet results) {
                MetadataOptions mo = new MetadataOptions(root, "bindings,formats,versions,execution");
                results.includeMetadata(mo.asArray());
            }
        };
    }
