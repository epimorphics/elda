...
public class APIModelLoader implements ModelLoader {
    ...
    @Override public Model loadModel(String uri) {
        ...
        Loader.log.info( "[%s]: loadModel '%s'", seqID, uri );
        if (uri.startsWith( Container.LOCAL_PREFIX )) {
            ...
            Loader.log.info( "[%s]: get TDB model '%s'", seqID, modelName );
            if (tdb.isEmpty()) Loader.log.warn( "[%s]: the TDB model at '%s' is empty -- has it been initialised?", seqID, modelName );
