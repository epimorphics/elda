...
public class ControlRestlet {
    ...
    public Response requestHandlerRDF(
            @PathParam("path") String pathstub,
            @Context UriInfo ui) {
        SpecRecord rec = lookupRequest(pathstub, ui);
        if (rec == null) {
    ...
    private void annotateEndpoints(Model spec, String baseURI) {
        ...
        while (ri.hasNext()) {
            ...
            ExtendedIterator<Statement> si = view.listProperties(API.uriTemplate); // .andThen(view.listProperties(API.uri));
            while (si.hasNext()) {
