...
public class run {
	...
	/**
	 * copy whatever the process <code>p</code> produces to standard output.
	...
	/**
	 * Build the args for the Jetty command line that will be launched when
	 * we've unzipped the jarfile. Any of <code>args</code> that start with -D
	 * will be inserted after the java command; any that don't will be inserted
	 * after start.jar.
	 */
	 ...
	 * <code>toDir</code>. Give up if there are any exceptions, which may trash
	 * existing files and leave incomplete new ones.
	 */
	public static void unzipJarfile(String toDir, String jarFile) throws IOException {
		...
		while (e.hasMoreElements()) {
			...
			if (x.isDirectory()) {
				// System.err.println( "creating directory " + x.getName() +
				// " in " + pathPrefix );
