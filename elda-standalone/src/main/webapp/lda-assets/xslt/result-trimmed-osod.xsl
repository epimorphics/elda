...
<xsl:template match="result" mode="script">
	...
	<xsl:if test="$showMap = 'true'">
		<!-- <script type="text/javascript" src="http://openspace.ordnancesurvey.co.uk/osmapapi/openspace.js?key={$openSpaceAPIkey}"></script> --> 
        <script type="text/javascript" src="http://ondemandapi.ordnancesurvey.co.uk/osmapapi/openspace.js?key=wec5muru"></script>
	</xsl:if>
...
//                          new OpenSpace.Control.CopyrightCollection(),
						...
						//var options = {controls: controls, products: ["OV0", "OV1", "OV2", "MSR", "MS", "CS04", "CS05", "VMDR", "VMD", "VMLR", "VML"], bgcolor: '0x9ecfff'};
                        var options = {controls: controls, products: ["OV0", "OV1", "OV2", "MSR", "MS", "CS04", "CS05", "CS06", "CS07", "CS08", "CS09", "CS10" ], bgcolor: '0x9ecfff'};
						// summaryMap = new OpenSpace.Map('map', {controls: controls});
                        summaryMap = new OpenSpace.Map('map', options);
			      ...
			      <xsl:for-each select="$markers">
				      ...
				      markers.addMarker(marker);
						...
						<xsl:for-each select="$markers">
                            ...
                            osMap = new OpenSpace.Map('<xsl:value-of select="concat('map', generate-id(.))"/>', options);
	...
	<section class="view">
		...
		<ul>
			<xsl:for-each select="hasVersion/item | hasVersion[not(item)]">
                ...
                <xsl:sort select="concat(./label, $view)" order="ascending"/>				<li>
					<xsl:apply-templates select="." mode="nav">
